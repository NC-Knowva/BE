...
const {
  formatTopicsSubjects,
  formatScoreboardGames,
  formatUsersGroup,
  formaCardPackTopics,
  formatStudyGroupSubjects,
} = require("./utils");
...
const seed = ({
  ...
  return db
    .query("DROP TABLE IF EXISTS scoreboard")
    .then(() => {
    ...
    .then((insertedSubject) => {
      ...
      const formattedInsertValues = formatTopicsSubjects(
        topics,
        subjectsInserted,
      ).map((topic) => {
    ...
    .then((tops) => {
      ...
      const formattedInsertValues = formatStudyGroupSubjects(
        study_group,
        subjectsInserted,
      ).map((study) => {
        return [
    ...
    .then((insertedGroups) => {
      const formattedInsertValues = formatUsersGroup(
        user_group_junction,
        insertedGroups.rows,
      ).map((users) => {
      ...
      const insertQuery = format(
        `insert into users_group_junction
    ...
    .then(() => {
      const formattedInsertValues = formaCardPackTopics(
        card_pack,
        topicsInserted,
      ).map((card) => {
        return [
    ...
    .then((gamesData) => {
      ...
      const formattedInsertValues = formatScoreboardGames(
        scoreboard,
        gamesData.rows,
      ).map((scoreb) => {
        return [
